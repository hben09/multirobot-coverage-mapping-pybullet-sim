...
class SubterraneanMapper:
    ...
    def plan_path_astar(self, start_grid, goal_grid):
        ...
        # Downsample path for smoother motion
        if len(path) > 5:
            path = path[::3]
            if path[-1] != goal_grid:
...
def main():
    ...
    # NEW: Direction bias tuning
    print("\n--- Direction Bias Settings ---")
    dir_weight_input = input("Direction bias weight (default=2.5, higher=more bias): ").strip()
    try:
        direction_weight = float(dir_weight_input)
    except:
    ...
    # Apply direction bias setting
    mapper.direction_bias_weight = direction_weight
