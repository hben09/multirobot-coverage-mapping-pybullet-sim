...
if NUMBA_AVAILABLE:
    ...
    @njit(cache=True)
    def _bresenham_collect_cells(x0, y0, x1, y1, out_cells, out_idx, max_cells):
        """
        Trace a ray and collect all cell coordinates (for dict sync).
    ...
    @njit(cache=True)
    def _process_rays_and_collect(grid, robot_gx, robot_gy, ray_endpoints_offset, 
                                   ray_endpoints_raw, ray_is_hit, 
                                   out_free_cells, out_obs_cells):
        """
        Process all rays AND collect touched cells for dict sync.
        
        Returns: (num_free_cells, num_obs_cells)
        """
        height, width = grid.shape
        ...
        for i in range(num_rays):
            # Offset coords for array indexing
            end_gx = ray_endpoints_offset[i, 0]
            end_gy = ray_endpoints_offset[i, 1]
            # Raw coords for dict keys
            end_raw_gx = ray_endpoints_raw[i, 0]
            end_raw_gy = ray_endpoints_raw[i, 1]
            is_hit = ray_is_hit[i]
            
            # Bresenham with cell collection
            x0, y0 = robot_gx, robot_gy
            x1, y1 = end_gx, end_gy
            
            # Calculate raw offset (difference between offset and raw coords)
            offset_x = ray_endpoints_offset[i, 0] - ray_endpoints_raw[i, 0]
            offset_y = ray_endpoints_offset[i, 1] - ray_endpoints_raw[i, 1]
            ...
            while True:
                # Update grid (with bounds check)
                if 0 <= x < width and 0 <= y < height:
                    if grid[y, x] != 2:
                        grid[y, x] = 1
                        # Collect free cell (raw coords)
                        if free_idx < max_free:
                            out_free_cells[free_idx, 0] = x - offset_x
                            out_free_cells[free_idx, 1] = y - offset_y
                            free_idx += 1
...
class NumbaOccupancyGrid:
    ...
    def update_from_lidar(self, robot_pos, scan_points, occupancy_dict, explored_set, obstacle_set):
        ...
        if NUMBA_AVAILABLE:
            ...
            endpoints_grid_raw = np.empty((n_points, 2), dtype=np.int32)
            for i in range(n_points):
                endpoints_grid_raw[i, 0] = int((endpoints_world[i, 0] - self.map_bounds['x_min']) * self.inv_resolution)
                endpoints_grid_raw[i, 1] = int((endpoints_world[i, 1] - self.map_bounds['y_min']) * self.inv_resolution)
        ...
        if NUMBA_AVAILABLE:
            ...
            self._update_python_fallback(
                robot_gx_raw, robot_gy_raw,
                scan_points,
                occupancy_dict, explored_set, obstacle_set
            )
        
        return n_points
    
    def _update_python_fallback(self, robot_gx, robot_gy, scan_points, 
                                 occupancy_dict, explored_set, obstacle_set):
        """Original pure-Python implementation as fallback."""
        x_min = self.map_bounds['x_min']
        y_min = self.map_bounds['y_min']
        inv_res = self.inv_resolution
        
        robot_grid = (robot_gx, robot_gy)
        occupancy_dict[robot_grid] = 1
        explored_set.add(robot_grid)
        
        for hit_x, hit_y, is_hit in scan_points:
            obs_gx = int((hit_x - x_min) * inv_res)
            obs_gy = int((hit_y - y_min) * inv_res)
            obstacle_grid = (obs_gx, obs_gy)
            
            if is_hit:
                occupancy_dict[obstacle_grid] = 2
                obstacle_set.add(obstacle_grid)
    ...
    def _sync_ray_paths_to_dict(self, robot_gx, robot_gy, endpoints_raw, is_hit,
                                 occupancy_dict, explored_set, obstacle_set):
        """
        Sync the ray-traced paths back to the dict/set structures.
        
        OPTIMIZATION: Instead of re-running Bresenham in Python, we extract
        the touched cells from the NumPy grid by scanning the region that
        could have been modified.
        """
        # Find bounding box of all rays
        min_gx = min(robot_gx, int(endpoints_raw[:, 0].min()))
        max_gx = max(robot_gx, int(endpoints_raw[:, 0].max()))
        min_gy = min(robot_gy, int(endpoints_raw[:, 1].min()))
        max_gy = max(robot_gy, int(endpoints_raw[:, 1].max()))
        
        # Convert to array indices
        arr_min_x = min_gx - self.grid_offset_x
        arr_max_x = max_gx - self.grid_offset_x
        arr_min_y = min_gy - self.grid_offset_y
        arr_max_y = max_gy - self.grid_offset_y
        
        # Clamp to grid bounds
        arr_min_x = max(0, arr_min_x)
        arr_min_y = max(0, arr_min_y)
        arr_max_x = min(self.width - 1, arr_max_x)
        arr_max_y = min(self.height - 1, arr_max_y)
        
        # Extract the subgrid that was potentially modified
        subgrid = self.grid[arr_min_y:arr_max_y+1, arr_min_x:arr_max_x+1]
        
        # Find all free cells (value 1) and obstacle cells (value 2)
        free_ys, free_xs = np.where(subgrid == 1)
        obs_ys, obs_xs = np.where(subgrid == 2)
        
        # Convert back to raw grid coordinates and update dicts
        for i in range(len(free_xs)):
            raw_gx = free_xs[i] + arr_min_x + self.grid_offset_x
            raw_gy = free_ys[i] + arr_min_y + self.grid_offset_y
            cell = (raw_gx, raw_gy)
            if cell not in obstacle_set:
                occupancy_dict[cell] = 1
                explored_set.add(cell)
        ...
        for i in range(len(obs_xs)):
            raw_gx = obs_xs[i] + arr_min_x + self.grid_offset_x
            raw_gy = obs_ys[i] + arr_min_y + self.grid_offset_y
            cell = (raw_gx, raw_gy)
            occupancy_dict[cell] = 2
            obstacle_set.add(cell)
    
    def _bresenham_python(self, x0, y0, x1, y1, mark_obstacle):
        """Pure Python fallback for Bresenham."""
