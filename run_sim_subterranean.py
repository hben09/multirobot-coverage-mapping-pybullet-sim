"""
Multi-robot coverage mapping simulation for subterranean environments
This version uses the SubterraneanEnvironment class to generate randomized cave/tunnel systems
"""
...
import os
from subterranean_env import SubterraneanEnvironment
...
class SubterraneanMapper:
    """Multi-robot mapper for subterranean environments"""

    def __init__(self, use_gui=True, env_type='tunnel', env_seed=None):
        """
        Initialize the mapper with a subterranean environment

        Args:
            use_gui: Whether to show PyBullet GUI
            env_type: Type of environment ('tunnel', 'cave', or 'maze')
        ...
        # Create subterranean environment
        self.env = SubterraneanEnvironment(seed=env_seed)
        ...
        # Get environment-specific bounds
        self.map_bounds = self.env.get_environment_bounds()
    ...
    def setup_environment(self):
        """Create the subterranean environment"""
        if self.env_type == 'tunnel':
            self.env.create_tunnel_network(num_tunnels=6, tunnel_length_range=(12, 20),
                                          tunnel_width_range=(4, 7))
        elif self.env_type == 'cave':
            self.env.create_cave_system(num_chambers=5, chamber_size_range=(10, 16))
        elif self.env_type == 'maze':
            self.env.create_maze_like_mine(grid_size=7, cell_size=6, wall_probability=0.3)
        else:
            print(f"Unknown environment type '{self.env_type}', defaulting to tunnel")
            self.env.create_tunnel_network()

    def create_robots(self):
        """Create robots at starting positions"""
        # Starting positions - all near origin
        start_positions = [
            [-2, 0, 0.25],   # Red robot
            [0, -2, 0.25],   # Green robot
            [2, 0, 0.25]     # Blue robot
        ]
    ...
    def setup_realtime_visualization(self):
        ...
        env_name = {'tunnel': 'Tunnel Network', 'cave': 'Cave System', 'maze': 'Mine Shaft'}
        title = f'Subterranean Mapping - {env_name.get(self.env_type, "Unknown")}\n(Click on grid map to control RED robot)'
    ...
    def run_simulation(self, steps=5000, scan_interval=10, use_gui=True, realtime_viz=True, viz_update_interval=50):
        """Run the simulation"""
        print("Starting subterranean mapping simulation...")
...
def main():
    """Main function with environment selection"""
    print("=" * 60)
    print("Multi-Robot Subterranean Coverage Mapping")
    print("=" * 60)
    print("\nAvailable environments:")
    print("1. Tunnel Network - Interconnected tunnels radiating from center")
    print("2. Cave System - Natural cave chambers with passages")
    print("3. Maze-like Mine - Grid-based mine shaft with random walls")

    choice = input("\nSelect environment type (1-3, default=1): ").strip()

    env_map = {'1': 'tunnel', '2': 'cave', '3': 'maze', '': 'tunnel'}
    env_type = env_map.get(choice, 'tunnel')
    ...
    mapper = SubterraneanMapper(use_gui=True, env_type=env_type, env_seed=env_seed)
